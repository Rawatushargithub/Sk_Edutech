...
const CourseForm = ({ mode }) => {
  ...
  useEffect(() => {
    ...
    if (!storedFranchiseId || !centerToken) {
      toast.error("Authentication required. Please login again.");
      navigate("/center-login"); // Redirect to login if no franchise ID or token
      return;
    }
    ...
    if (isEditMode && courseId) {
      ...
      fetch(`${API_BASE_URL}/api/v1/institute_courses/course/${courseId}`, {
          ...
          if (!res.ok) {
            ...
            return res
              ...
              .catch(() => {
                ...
                throw new Error(
                  "Failed to fetch course details. Status: " + res.status
                );
              });
          }
          return res.json();
        ...
        .then((courseData) => {
          // Verify that this course belongs to the current franchise
          if (courseData.franchiseId !== storedFranchiseId) {
  ...
  const handleSubmit = async (e) => {
    ...
    notes.forEach((note) => {
      if (note.type === "link" && note.title && note.url) {
        ...
        if (note.file) {
          // A new file has been selected for this note slot
          materialsToSubmit.push({
