...
const AddCoursebyAdmin = ({ mode }) => {
  ...
  useEffect(() => {
    ...
    setFranchiseId("Admin"); // ✅ Always set to Admin
    ...
    if (isEditMode && courseId) {
      ...
      fetch(`${API_BASE_URL}/api/v1/institute_courses/course/${courseId}`, {
        ...
        .then((courseData) => {
          // ✅ forcefully allow Admin (skip franchise ownership check)
          setFormData({
          ...
          setCourseVideoLinks(
            courseData.courseVideoLinks?.length
          ...
          setNotes(
            courseData.courseMaterials?.length
  ...
  const removeVideoLink = (idToRemove) => {
    ...
    setCourseVideoLinks(
      updated.length ? updated : [{ id: Date.now(), title: "", link: "" }]
    );
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // 🔹 Force these two values regardless of local storage
    submissionData.append("franchiseId", "Admin");
    submissionData.append("byAdmin", true);
    submissionData.append("adminApprovalStatus", "approved");
    ...
    else if (isEditMode && existingCourseImage) {
      submissionData.append("existingCourseImage", existingCourseImage);
    }

    const materialsToSubmit = [];
    const materialFilesToUpload = [];
    ...
    notes.forEach((note) => {
      if (note.type === "link" && note.title && note.url) {
        materialsToSubmit.push({
          ...
          isNew: note.isNew,
          _id: !note.isNew ? note._id : undefined,
        });
        ...
        if (note.file) {
          materialsToSubmit.push({
            ...
            type: "file",
            originalFileNamePlaceholder: note.file.name,
            isNewFile: true,
          });
          materialFilesToUpload.push(note.file);
        } else if (!note.isNew && note.url) {
          materialsToSubmit.push({
            title: note.title,
            type: "file",
            url: note.url,
    ...
    materialFilesToUpload.forEach((file) =>
      submissionData.append("courseMaterialFiles", file)
    );

    const url = isEditMode
      ? `${API_BASE_URL}/api/v1/institute_courses/update/${courseId}`
      : `${API_BASE_URL}/api/v1/institute_courses/createCourse`;
    ...
    try {
      const response = await fetch(url, {
        ...
        body: submissionData,
        headers: {
          // You may still need admin token if backend requires it
  ...
  const inputBaseStyle =
    "mt-1 block w-full px-3 py-2 rounded-md border border-gray-300 shadow-sm sm:text-sm focus:outline-none";
            ...
            <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-6">
                ...
                <div>
                  ...
                  >
                    ...
                    <span className="text-red-500 text-xs ml-2 font-normal">
                      (Use full form — Will be printed on certificate)
                    </span>
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={handleInputChange}
                    placeholder="For ex : Machine Learning"
                    required
                    className={
                ...
                <div>
                  ...
                  >
                    Course Subject{" "}
                    <span className="text-red-500 text-xs ml-2 font-normal">
                      (Enter carefully — Will be printed on certificate)
                    </span>
                  </label>
                  <input
                    type="text"
                    ...
                    onChange={handleInputChange}
                    placeholder="For ex : MS Excel , MS Word , MS Powerpoint"
                    required
                    className={
                      isEditMode ? disabledInputStyle : noRingInputStyle
                    }
              ...
              <div className="space-y-4 border-t border-gray-200 pt-6">
                <h3 className="text-lg font-medium text-gray-900">
                  Course Video Links (Optional)
                </h3>
                {courseVideoLinks.map((video, index) => (
                  <div
                    key={video.id}
                    className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center p-3 bg-gray-50 rounded-md"
                  >
                    <input
                      type="text"
                      name="title"
                      data-index={index}
                      value={video.title}
                      onChange={(e) => handleVideoLinkChange(index, e)}
                      className={`${noRingInputStyle} md:col-span-1`}
                    ...
                    <button
                      ...
                      className={`${dangerButtonStyle} h-10 flex items-center justify-center md:col-span-1`}
                    >
              ...
              <div className="border-t border-gray-200 pt-6">
                ...
                <input
                  ...
                  accept="image/*"
                  className={`mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 ${noRingInputStyle}`}
                />
                {courseImagePreview && (
                  <img
                ...
                {!courseImagePreview && existingCourseImage && (
                  <img
                    src={existingCourseImage}
                    alt="Current Course"
                    className="mt-2 h-32 w-auto rounded-md object-cover"
                  />
                )}
              ...
              <div className="space-y-4 border-t border-gray-200 pt-6">
                <h3 className="text-lg font-medium text-gray-900">
                  Course Notes / Materials (Optional)
                </h3>
                {notes.map((note, index) => (
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                      ...
                      <button
                        type="button"
                        onClick={() => removeNoteInput(note.id)}
                        className={`${dangerButtonStyle} h-10 flex items-center justify-center`}
                      >
                      ...
                      <div>
                        {note.isNew || !note.url ? (
                          <input
                            ...
                            className={`block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-slate-100 file:text-slate-700 hover:file:bg-slate-200 ${noRingInputStyle}`}
                          />
                        ) : (
                          <div className="text-sm text-gray-600 p-2 border border-dashed border-gray-300 rounded-md">
                            ...
                            <input
                              ...
                              onChange={(e) => handleNoteChange(index, e)}
                              accept=".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png"
                              className={`mt-2 block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:font-semibold file:bg-slate-50 file:text-slate-600 hover:file:bg-slate-100 ${noRingInputStyle}`}
                            />
                          </div>
                        )}
                        {note.file && note.isNew && (
                          <p className="text-xs text-green-600 mt-1">
                            New file selected: {note.fileName}
                          </p>
                        )}
                      </div>
                    )}
              ...
              <div className="border-t border-gray-200 pt-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                  <label
                    htmlFor="instituteStatus"
                    className="block text-sm font-medium text-gray-700 mb-1 sm:mb-0"
                  >
                    Institute Status
                  </label>
                  <select
                    ...
                    id="instituteStatus"
                    value={formData.instituteStatus}
                    onChange={handleInputChange}
                    className={`${noRingInputStyle} sm:w-auto`}
                  >
                ...
                <p className="mt-1 text-xs text-gray-500">
                  Set whether this course is currently active for your
                  institute.
                </p>
              </div>
              ...
              <div className="flex justify-end space-x-3 border-t border-gray-200 pt-6">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className={`${primaryButtonStyle} min-w-[120px]`}
                >
                  {loading ? (
                    <svg
