...
const AddNote = ({ notes = [], setNotes = () => {} }) => { // Default values
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await axios.post("http://localhost:8000/api/v1/institute_note", formData, {
    ...
    <div className="min-h-screen p-6 bg-gray-100">
      ...
      <div className="bg-white p-6 shadow-md rounded">
        ...
        <div className="flex space-x-4">
          ...
          <button onClick={() => navigate("/institute/Notes")} className="bg-red-500 text-white px-4 py-2 rounded">Cancel</button>
        </div>
      </div>
    </div>
  );
