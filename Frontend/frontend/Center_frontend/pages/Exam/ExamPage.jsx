...
const ExamManagement = () => { 
  ...
  const markAsDone = async (id) => {
    try {
      const response = await fetch(`http://localhost:8000/api/v1/institute_exam/exams/${id}/status`, {  // ✅ Fix endpoint
        method: "PUT",
  ...
  const deleteExam = async (id) => {
    try {
      const response = await fetch(`http://localhost:8000/api/v1/institute_exam/exams/${id}`, {
        method: "DELETE",
      });
