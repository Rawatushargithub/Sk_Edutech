            ...
            {modalStep === 'form' && (
                <form onSubmit={handleSubmit(handleProceedToOtp)} className="space-y-8">
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-6">
                            <div>
                                <label htmlFor="franchiseLogo" className={labelClass}>Franchise Logo <span className="text-gray-500">10 KB – 2 MB</span> <span className="text-red-500">*</span></label>
                                <input type="file" id="franchiseLogo" {...register("franchiseLogo", { required: "Franchise logo is required", validate: { size: (files) => { const f = files?.[0]; if (!f) return true; const min = 10 * 1024; const max = 2 * 1024 * 1024; return (f.size >= min && f.size <= max) || "File size must be between 10 KB and 2 MB"; } } })} onChange={(e) => handleInstantFileCheck(e, 'franchiseLogo', { minBytes: 10 * 1024, maxBytes: 2 * 1024 * 1024, allowedMimes: ['image/jpeg','image/png','image/jpg'], label: 'Franchise Logo' })} className="mt-2 block w-full px-4 py-3 bg-white border-2 border-dashed border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base transition duration-200 ease-in-out hover:border-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept="image/jpeg,image/png,image/jpg" />
                            ...
                            <div>
                                <label htmlFor="franchiseSignature" className={labelClass}>Franchise Signature <span className="text-gray-500">10 KB – 2 MB</span> <span className="text-red-500">*</span></label>
                                <input type="file" id="franchiseSignature" {...register("franchiseSignature", { required: "Franchise signature is required", validate: { size: (files) => { const f = files?.[0]; if (!f) return true; const min = 10 * 1024; const max = 2 * 1024 * 1024; return (f.size >= min && f.size <= max) || "File size must be between 10 KB and 2 MB"; } } })} onChange={(e) => handleInstantFileCheck(e, 'franchiseSignature', { minBytes: 10 * 1024, maxBytes: 2 * 1024 * 1024, allowedMimes: ['image/jpeg','image/png','image/jpg'], label: 'Franchise Signature' })} className="mt-2 block w-full px-4 py-3 bg-white border-2 border-dashed border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base transition duration-200 ease-in-out hover:border-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" accept="image/jpeg,image/png,image/jpg" />
