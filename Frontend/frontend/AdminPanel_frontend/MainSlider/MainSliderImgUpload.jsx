...
const MainSliderUploadPage = () => {
  ...
  const fetchImagesFromDatabase = async () => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/api/v1/mainSliderImages/`
      );
      setSliderImages(response.data);
    } catch (error) {
      console.error("Error fetching images from database:", error);
      setErrorMessage(
        "Failed to load existing images. Please refresh the page."
      );
    }
  };
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      const newPendingImages = files.map((file) => {
        ...
        reader.onload = (e) => {
          setPendingImages((prevImages) =>
            prevImages.map((img) =>
              img.id === fileId ? { ...img, url: e.target.result } : img
            )
          );
        };
      ...
      const response = await axios.post(
        `${API_BASE_URL}/api/v1/mainSliderImages/upload`,
        formData,
        {
          headers: {
      ...
      setTemporaryServerImages((prev) => [
        ...prev,
        ...response.data.tempImages,
      ]);
  ...
  const handleDeletePendingImage = (id) => {
    ...
    if (pendingIndex !== -1) {
      ...
      if (updatedTempServerImages[pendingIndex]) {
        ...
        axios
          .post(`${API_BASE_URL}/api/v1/mainSliderImages/cleanup-temp`, {
  ...
  const handleReorder = async (id, direction, isPending) => {
    if (isPending) {
      ...
      if (temporaryServerImages.length > 0) {
        ...
        updatedTempServerImages[currentIndex] =
          updatedTempServerImages[newIndex];
  ...
  const handleSaveToCloudinary = async () => {
    ...
    try {
      ...
      if (temporaryServerImages.length > 0) {
        const response = await axios.post(
          `${API_BASE_URL}/api/v1/mainSliderImages/publish-with-upload`,
          {
            tempImages: temporaryServerImages,
          }
        );
      ...
      setErrorMessage(
        error.response?.data?.message ||
          "Failed to publish images. Please try again."
      );
      setUploading(false);
    }
  };
  ...
  const totalSize = [...sliderImages, ...pendingImages].reduce(
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-black to-gray-800 px-6 py-4">
            <h1 className="text-2xl font-bold text-white">
              Homepage Main Slider Management
            </h1>
            <p className="text-blue-100 mt-1">
              Upload and manage your homepage slider images (maximum 10)
            </p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  You can upload up to {10 - allImages.length} more image
                  {10 - allImages.length !== 1 ? "s" : ""}
                </p>
                {pendingImages.length > 0 && (
                  <p className="text-sm text-orange-500 mt-1">
                    You have {pendingImages.length} pending image
                    {pendingImages.length !== 1 ? "s" : ""} to publish
                  </p>
                )}
              </div>
          ...
          <div className="px-6 py-4">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-medium text-gray-900">
                Current Slider Images ({allImages.length}/10)
              </h2>
            ...
            {allImages.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between mt-3">
                        <div className="space-x-1">
                          <button
                            onClick={() =>
                              handleReorder(image._id, "up", false)
                            }
                            disabled={index === 0}
                          ...
                          <button
                            onClick={() =>
                              handleReorder(image._id, "down", false)
                            }
                ...
                {pendingImages.map((image, index) => (
                  <div
                    key={image.id}
                    className="border rounded-lg overflow-hidden bg-white shadow-sm border-orange-300"
                  >
                    <div className="relative aspect-w-16 aspect-h-9 bg-gray-100">
                      <img
                        src={image.url}
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex justify-between mt-3">
                        <div className="space-x-1">
                          ...
                          <button
                            onClick={() =>
                              handleReorder(image.id, "down", true)
                            }
          ...
          <div className="mt-8 bg-white shadow-md rounded-lg overflow-hidden">
            <div className="bg-gradient-to-r from-gray-700 to-gray-900 px-6 py-4">
              ...
              <p className="text-gray-300 mt-1">
                This is how your slider will appear on the homepage
              </p>
            </div>
            ...
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <div className="flex justify-end space-x-3">
                <button
                  ...
                  className="px-4 py-2 text-gray-600 border border-gray-300 rounded hover:bg-gray-200 cursor-pointer "
                >
                  Cancel
                </button>
                <button
